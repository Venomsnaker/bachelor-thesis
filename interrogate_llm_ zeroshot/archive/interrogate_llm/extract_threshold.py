...
def evaluate_threshold(gt, cosine_scores):
    ...
    y_predict_roc = cosine_scores > best_threshold_roc
    best_accuracy_roc = accuracy_score(gt, y_predict_roc)
    ...
    y_predict_pr = cosine_scores > best_threshold_pr
    best_accuracy_pr = accuracy_score(gt, y_predict_pr)

    # F1 Score Maximization
    best_threshold_f1 = thresholds_pr[np.argmax(f1_scores)]
    y_predict_f1 = cosine_scores > best_threshold_f1
    best_accuracy_f1 = accuracy_score(gt, y_predict_f1)
...
def create_sentence_pairs_embeddings(embedding_model, samples):
...
if __name__ == '__main__':
    ...
    embedding_models = embedding_models[args.embedding_model_name]
    gt, cosine_scores = create_sentence_pairs_embeddings(embedding_models, samples=args.samples)
